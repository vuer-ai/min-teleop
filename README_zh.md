...
使用 [Vuer](https://github.com/vuer-ai/vuer) 的最小化 VR 遥操作示例。通过浏览器中的 VR 头显控制仿真（MuJoCo）或真实机器人。
...
也可以直接用 pip 安装（参见 `environment.yml` 中的 `pip` 部分获取包列表）。

### 2. 生成自签名 SSL 证书

Vuer 通过 HTTPS 提供服务（WebRTC 和浏览器安全 API 要求）。本地开发需要自签名证书：
...
从 VR 头显连接时，需要先在头显浏览器中打开 `https://<你的IP>:8012` 并接受证书警告。

### 3. 运行示例

**本地 Viewer** — 最简单，弹出桌面 MuJoCo 窗口：

```bash
# Linux
python examples/local_viewer/main.py

# macOS（MuJoCo 需要主线程访问）
mjpython examples/local_viewer/main.py
```

**浏览器 3D** — 在 VR 头显中渲染完整 3D 场景：

```bash
python examples/browser_3d/main.py
```

**WebRTC 单进程** — 将渲染视频串流到 VR：

```bash
python examples/webrtc/simple/main.py
```

**WebRTC 双进程** — 将 Vuer 前端与仿真分离，性能更好：
...
每个示例会打印一个 URL（如 `https://192.168.x.x:8012`）。在 VR 头显浏览器或任意桌面浏览器中打开。

### 4.（可选）G1 + IsaacLab 示例

`examples/webrtc/external_sim/` 示例驱动 Unitree G1 机器人在 IsaacLab 中运行，需要额外依赖：

```bash
# 通过 conda-forge 安装 pinocchio
conda install -c conda-forge pinocchio

# 安装 G1 专用 pip 包
pip install casadi pyyaml nlopt anytree pytransform3d trimesh lxml

# 从子模块安装 dex-retargeting
git submodule update --init
pip install --no-deps -e third_party/dex-retargeting

# unitree_sdk2py 需要单独安装（未发布到 PyPI）
# pip install -e <unitree_sdk2_python 的路径>
```
...
本节介绍使用 Vuer 构建 VR 遥操作系统的三种架构方案。从最简单的开始，逐步解决更复杂的需求。
...
**适用场景**：开发调试，验证控制链路；控制真实机器人时用眼睛直接看实物；不需要在 VR 内看到反馈。
...
进程间通讯通过 `transport.py` 封装的 ZMQ PUSH/PULL 完成：
...
> **⚠ 自签名证书**：浏览器需要直接访问 Process 2（`:8013`）的 HTTPS 服务来建立 WebRTC 连接。由于使用自签名证书，浏览器会静默拒绝连接（视频黑屏，无报错）。**首次使用前，需要在浏览器中手动打开 `https://<sim_host>:8013` 并接受证书安全警告**。
...
- **server 端**的 ZMQ 应绑定 `0.0.0.0`（而非 `127.0.0.1`）
- **client 端**默认使用 `app.local_ip`（局域网 IP）。如果 server 在其他机器上：
  ```bash
  SIM_HOST=192.168.1.100 python examples/webrtc/dual_process/client.py
  ```
- `WebRTCVideoPlane(src=...)` 的 URL 必须是浏览器可达的地址，不能是 `127.0.0.1`

**2. 自签名 SSL 证书**

**在 VR 头显的浏览器中，需要手动访问每个 HTTPS 端点并接受证书警告**：

- 方案 2a：访问 `https://<host>:8012`
- 方案 2b：需要分别接受 `https://<host>:8012` 和 `https://<host>:8013`
- 方案 2c：还需要接受 IsaacLab WebRTC 服务器的证书
...
| 带宽极低（几百字节 vs 几十 KB/帧） | 其他仿真器/真机需要状态 → qpos 映射 |

**适用场景**：需要低延迟、高画质、3D 立体的 VR 体验；VR 设备有足够算力（Quest 3、Pico 4 等）；真实机器人 + 对应 MJCF 模型做"数字孪生"。
